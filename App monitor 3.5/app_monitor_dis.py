...
class app_monitor():
    ...
    def rtorrent_monitor(self, Web_Hook_URL):
        ...
        if Pid == 256:  # Pid is 256 when os.system doesn't give O/P for Linux
            ...
            data = {
                "content": '**rtorrent application was down and has been restarted by script** :)'}
        ...
        if Pid2 == 256:  # no effect of restart so time to repair
            ...
            data = {
                "content": '**rtorrent application was down and has been repair by script** :)'}
            response = requests.post(Web_Hook_URL, json=data)
        ...
        if final_pid == 256:  # restart or repair comands doesn't work
            data = {"content": ' **Script is unable to FIX your rTorrent so please open a support ticket from here - https://my.ultraseedbox.com/submitticket.php**'}
            response = requests.post(Web_Hook_URL, json=data)
        else:
            pass
    ...
    def docker_app(self, apps, Web_Hook_URL):
        for i in apps:
            ...
            if count <= 2:
                ...
                data = {
                    "content": '**Your {} application was down and has been restarted by script** :)'.format(i)}
                response = requests.post(Web_Hook_URL, json=data)
            else:
                pass
            time.sleep(180)
            ...
            count = len(status.splitlines())
            if count <= 2:
                data = {"content": "**Script is unable to FIX your {} so please open a support ticket from here - https://my.ultraseedbox.com/submitticket.php**".format(i)}
                response = requests.post(Web_Hook_URL, json=data)
